...
mapit = function (options)
{
    ...
    var _buildHtml = function (container)
    {
        ...
        acc += "<div id='" + controlIds.contentId + "' class='mapit-content row mx-0'>";
        acc += _buildAccordion(container);
        acc += "<div id='" + controlIds.mapContentId + "' class='mapit-map-content col-md-9 px-0'>";
        ...
        if (_opts.accordionActive)
        {
            jQuery("#" + controlIds.accordionId).accordion({
                fillSpace: true,
                animated: _opts.accordionAnimation,
                active: defaultCat,
                change: function (event, ui) { console.log(ui.newHeader); _showCategory(ui.newHeader.context.getAttribute("catid")); }
            });
        }
    ...
    var _buildAccordion = function ()
    {
        ...
        acc += "<div id='" + controlIds.navId + "' class='mapit-navigation col-md-3 pr-0 py-3'>"
        ...
        acc += "<div id='" + controlIds.accordionContainerId + "' style='overflow: hidden' >";
        acc += "<div class='mapit-accordion js-scrollbar-accordion' data-theme='dark' style='height: 500px;' id='" + controlIds.accordionId + "'>"
        ...
        jQuery.each(_categories, function (catIndex, category)
        {
            ...
            acc += "<h3 class='ville-title' id='" + eleId + "' catid='" + catId + "'><a href='javascript:void(0);'>" + category.get_name() + "</a></h3>";
            acc += "<div>";
            ...
            jQuery.each(locations, function (locIndex, location)
            {
                ...
                acc += "<div class='plase-list pt-2 pb-3'>";
                acc += "<a class='ville-place' data-slug='"+ location.get_slug() +"' href='javascript:void(0);' id='" + eleId + "' locid='" + locId + "' name='loc' class='mapit-accordion-link'>" + locName + "</a>";
                acc += "<span class='vill-desc'>"+location.get_address()+"<span>";
                acc += "</div>";
    ...
    var GMAP = {
        ...
        showContent: function (location, isSwitchingCategory)
        {
            ...
            else // use the default html
            {
                ...
                var latLng = { lat: cords[0], lng: cords[1] };
                var address = location.get_address() + ', ' + ( location.get_city() ?  location.get_city() : ''  ) + ', ' + ( location.get_state() ?  location.get_state() : ''  ) + ' ' + ( location.get_zip() ?  location.get_zip() : ''  ) + (_opts.coordinatesInDirections ? "@" + latLng.lat + "," + latLng.lng : "");
                var displayAddress = location.get_address() + '<br/>' +( location.get_city() ?  location.get_city() : ''  ) + ', ' +( location.get_state() ?  location.get_state() : ''  ) + ' ' + ( location.get_zip() ?  location.get_zip() : ''  ) + (_opts.bubbleShowCoordinates ? "<br/>" + location.get_cords() : "");
...
mapit.location = function ()
{
    ...
    this.get_slug = function ()
    {
        return this.getProperty("slug");
    };
    this.set_slug = function (val)
    {
        this.setProperty("slug", val);
    }
    //#endregion
